...
function Suppliers () {
    ...
    const handleValidation = async (supplierId: number, currentStatus: boolean, activeStatus: boolean) => {
        const newValidationStatus = !currentStatus;
        const newIsActive = !activeStatus;
    
        try {
            const response = await axios.put(`https://localhost:7017/Users/validateSupplier/${supplierId}`, { isValidate: newValidationStatus, isActive: newIsActive }, {
                    headers: {
    ...
    <div className='view-supp-header'>
        ...
        <h2 style={{ borderRight:'solid 2px', padding:'15px' }}>
            ...
            <div className='status-filter-container'>
                <h3 style={{ marginRight:'30px', color:'#020654' }}
                >
                    Sort by
                </h3>
    ...
    <tbody>
        {filteredSuppliers.map(supplierItem => (
            ...
            <td>
                ...
                >
                    <img className='validation-icon' src={supplierItem.isValidate ? validIcon : invalidIcon }/>
                ...
                <div className="modal-body">
                    {selectedSupplier && (
                        <div className='supplier-profile-container'>
                            <img className='shop-profile-img' src={`https://localhost:7017/${selectedSupplier.image}`} alt="supplier profile picture" />
                            <div className='supplier-text-info'>
                                <h1 style={{ marginBottom:'20px' }}>{selectedSupplier.shopName}</h1>
                                <div className='supplier-info-container'>
                                    ...
                                    <div className='supplier-text-2'>
                                        ...
                                        <h5 className='info-2-supp'>{selectedSupplier.isActive ? "Active" : "Not Active"}</h5>
                                        <h5 className='info-2-supp'>{selectedSupplier.address}</h5>
                                        <h5 className='info-2-supp'>{selectedSupplier.email}</h5>
                                        <h5 className='info-2-supp'>{selectedSupplier.phoneNumber}</h5>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="collapse" id="collapseChangePass" style={{ marginRight:'15px' }}>
                                <div className="card card-body">
                                    ...
                                    <div className='saveChanges-btn-container'>
                                        <button className='saveChanges-btn' onClick={() => handleUpdatePassword(selectedSupplier.id)}>Save Changes</button>
                                    </div>
                                </div>
                            </div>
                        )}
