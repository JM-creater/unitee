...
function Shop() {
        ...
        const fetchAverageRatingProduct = async () => {
            try {
                ...
                const ratingsPromises = supplierResponse.data.map((supplier) =>
                    axios.get(
                        `https://localhost:7017/Rating/average-product-rating/${supplier.id}`
                    )
                );
  ...
  // * Windows Event Listener Focus
    ...
    // * Get the overall recommend products
    useEffect(() => {
        const fetchRecommendedOverAll = async () => {
            try {
                const response = await axios.get(`https://localhost:7017/Product/recommendedOverAll?userId=${userId}`);
                setRecommendedOverAll(response.data);
            } catch (error) {
                console.error(error);
            }
        }
        fetchRecommendedOverAll();
    }, [userId]);
